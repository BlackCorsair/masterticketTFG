...
public class filterevents extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String condiciones = "(categoria LIKE '%"+categoria+"%')";
		
		if(titulo != null){
			condiciones += "AND (titulo LIKE '%"+titulo+"%')";
		}
		
		if(lugar != null){
			condiciones += "AND (titulo LIKE '%"+lugar+"%')";
		}
		
		if(estado != null){
			condiciones += "AND (estado LIKE '%"+estado+"%')";
		}
		...
		try{
	        ...
	        if(sql_date != null){
				condiciones += "AND (fecha ="+sql_date+")";
			}
	    ...
	    bd.openConnection();
	    ResultSet rs = bd.exQuery("SELECT * FROM eventos WHERE "+condiciones , "SELECT");
